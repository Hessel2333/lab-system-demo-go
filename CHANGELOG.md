...
## [2026-02-28] - 用户与组织权限页架构重构（第五轮）

### ✅ 用户查询口径升级（后端）

- `GetUsers` 增加组织范围参数：
  - `include_children=true`（默认）：按所选组织节点 + 全部下级节点查询
  - `include_children=false`：仅查询直属成员
- 新增 `collectDeptScopeIDs` 递归收集组织范围，解决“选中上级组织看不到成员”的可用性问题。
- 用户创建/编辑新增角色白名单校验（后端），拒绝非法角色写入，避免权限模型漂移。

### ✅ 用户管理页结构重构（前端）

- `UserManagement.vue` 改为三层结构：
  - 左侧组织树（选择范围）
  - 中部统一台账（`TableSection + LedgerTable`）
  - 行内动作（权限/编辑/删除）常驻可见
- 工具条新增“含下级/仅直属”范围开关，并与后端查询联动。
- 新增试剂双签全局持有人摘要（A/B）与角色筛选，支持快速识别权限冲突风险。
- 成员台账新增“流程角色”列，直接展示钥匙持有人 A/B 标记。

### ✅ 成员编辑与权限弹窗升级（前端）

- `UserDialog.vue`：
  - 新增“所属组织”可选项（树结构扁平化展示），编辑成员不再受当前树节点限制。
- `UserPermissionDialog.vue`：
  - 去除占位型无效 Tab，收敛为“仪器预约权限 / 试剂流程权限”双主场景；
  - 增加用户身份摘要（姓名/角色/组织）；
  - 试剂流程权限新增全局持有人 A/B 当前值展示；
  - 统一改用 `toast` 反馈，不再使用 `alert`。

### ✅ 接口适配

- `fetchUsers` 支持 `include_children` 参数，前后端查询口径一致。
